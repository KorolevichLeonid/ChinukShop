...
public class CartActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        productAdapter = new ProductAdapter(this, cartItems, true);
    ...
    private void loadCartItems() {
        ...
        if (currentUser != null) {
            for (String productId : currentUser.getCartItems()) {
                // Загружаем информ о продукте по ID из бд
    ...
    private void checkout() {
        ...
        if (currentUser != null) {
            ...
            orderDetails.put("orderItems", cartItemsIds);
            ...
            ordersRef.push().setValue(orderDetails).addOnCompleteListener(task -> {
                if (task.isSuccessful()) {
                    ...
                    Toast.makeText(CartActivity.this, "Ошибка оформления заказа: " + task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                }
            });
        } else {
    ...
    public void removeFromCart(Product product) {// Удаление ID продукта из списка
        Users currentUser = Prevalent.getCurrentOnlineUser();
        if (currentUser != null) {
